...
function NextMatchCard({ m, featured, daysLeft, onPredict }) {
  ...
  const Side = ({ name, code }) => (
    <div onClick={() => { if (code && window.MB_openTeamByCode) { window.MB_openTeamByCode(code); } else if (window.__mbOpenTeamByName) window.__mbOpenTeamByName(name); }} className="mb-press" title={`Ver ${name}`} style={{ flex: 1, textAlign: 'center', minWidth: 0, cursor: 'pointer' }}>
