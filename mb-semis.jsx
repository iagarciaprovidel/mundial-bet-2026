        ...
        <React.Fragment>
          ...
          <div onClick={canEdit ? () => setOpen(true) : undefined} className={canEdit ? 'mb-press mb-card-hover' : ''}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 'var(--t-2xs)', color: locked ? 'var(--muted)' : '#C4A0FF' }}>
              ...
              {myPick ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 3 }}>
                  {myPick.map((c) => <img key={c} src={`https://flagcdn.com/h40/${c}.png`} alt={c} style={{ height: 14, width: 'auto', borderRadius: 3, boxShadow: '0 1px 4px rgba(0,0,0,0.5)' }} />)}
                  {canEdit && <span style={{ fontSize: 8.5, color: '#9B6DFF', fontWeight: 700, marginLeft: 3, flexShrink: 0 }}>· Cambiar</span>}
                </div>
              ) : (
                <div style={{ fontSize: 8.5, color: 'rgba(155,109,255,0.7)', marginTop: 1 }}>Elige 4 de los 8 clasificados · +{fmt(PTS_PER * 4)} pts máximo</div>
              )}
            </div>
            {canEdit && !myPick && <span style={{ fontSize: 14, color: '#9B6DFF', flexShrink: 0 }}>→</span>}
          </div>
        </React.Fragment>
      );
    }
