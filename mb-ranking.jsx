...
(function () {
  ...
  // Medallas ACUMULABLES por nº de apuestas: +🥉10 · +🥈25 · +🥇50.
  function badgeIcons(n) { return (!n || n < 10) ? '' : '🥉' + (n >= 25 ? '🥈' : '') + (n >= 50 ? '🥇' : ''); }
  // Subtexto: medallas + cantidad + "apuestas", todo junto y chico (se relacionan).
  function betText(n) { if (!n || n < 1) return ''; const m = badgeIcons(n); return ' · ' + (m ? m + ' ' : '') + n + ' apuestas'; }
  ...
  function RankingReal({ compact, limit }) {
    ...
    return (
      <div>
        {!compact && <div style={{ fontSize: 'var(--t-2xs)', color: 'var(--muted-2)', textAlign: 'center', marginBottom: 10 }}>Cada jugador parte con 90.000 puntos · 🥉🥈🥇 = 10/25/50 apuestas</div>}
          ...
          {shown.map((u, i) => {
            ...
            return (
              <div key={u.uid} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 8px', borderRadius: 'var(--r-sm)', background: isMe ? 'rgba(212,175,55,0.14)' : 'transparent', borderBottom: i < shown.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                ...
                <span onClick={() => u.champion && window.MB_openTeam && window.MB_openTeam(u.champion)} style={{ cursor: u.champion ? 'pointer' : 'default', flexShrink: 0 }}>
            ...
            return (
              <div key={u.uid} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 2px', borderRadius: 'var(--r-sm)', background: isMe ? 'rgba(212,175,55,0.10)' : 'transparent', borderBottom: i < players.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 9, color: 'var(--muted-2)' }}>{u.groupName ? '👥 ' + u.groupName : (u.noGroup ? 'Individual' : 'Sin equipo')}{betText(typeof u.betsCount === 'number' ? u.betsCount : 0)}</div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div className="num" style={{ color: 'var(--gold-light)', fontWeight: 700, fontSize: 'var(--t-sm)', whiteSpace: 'nowrap' }}>{fmt(saldoOf(u))}<Arrow cur={saldoOf(u)} prev={SALDO_INICIAL} /></div>
